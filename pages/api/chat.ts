...
// Функция для удаления Markdown форма��ирования
function cleanMarkdown(text: string): string {
  return text
    ...
    .replace(/^\*\s+/gm, '• ')
    // Убираем лишние звездочки (но сохраняем эмод��и)
    .replace(/\*(?![^\s])/g, '')
...
- ��АПРЕЩЕНО использовать слова на английском языке в ответах (например: "code", "web", "frontend", "backend", "API" и т.д.)
