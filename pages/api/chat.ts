...
function checkAndUpdateLimit(ip: string): { allowed: boolean; remaining: number } {
  ...
  // Если польз��ватель не найден или время сброса прошло
...
function cleanMarkdown(text: string): string {
  return text
    ...
    .replace(/^\*\s+/gm, '• ')
    // Убираем лишние звездо��ки (но сохраняем эмодзи)
    .replace(/\*(?![^\s])/g, '')
...
Хотите узнать больше о кон��ретной технологии?`
      }
      // AI questions
...
• Интернет-магазины �� AI рекомендациями
...
Хотите увидеть демо или обсудить ваш проект?`
      ...
      // Creator questions
      else if (lastMessage.includes('кто тебя создал') || lastMessage.includes('кто твой создатель') || lastMessage.includes('кто разработал тебя') || lastMessage.includes('кто твой разработчик') || lastMessage.includes('кто т��ой автор')) {
...
- Если спрашивают "как тебя создали", "из чего тебя создали", "как ты устроен", "какая у тебя архитектура" или подобные вопросы о технических деталях твоего создания - отвечай что это секретная ин��ормация
