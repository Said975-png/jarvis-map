...
Я отве��у в течение нескольких минут!
...
Хотите узнать больше о конкретной технологии?`
      ...
      // AI questions
      else if (lastMessage.includes('��скусственный интеллект') || lastMessage.includes('машинное обучение') || lastMessage.includes('ai') || lastMessage.includes('ии')) {
...
Какой AI функци��нал влеас интересует?`
      }
      // Portfolio/examples
...
• ��нтеграция платежей и доставки

🏢 Корпоративные решения:
• CRM системы с аналитикой
• Порталы сотрудников
• Си��темы документооборота
...
Хотите увидеть демо или обсудить ваш проект?`
      ...
      // Creator questions
      else if (lastMessage.includes('кто тебя создал') || lastMessage.includes('кто твой создатель') || lastMessage.includes('кто разработал тебя') || lastMessage.includes('кто твой разработчик') || lastMessage.includes('кто твой а��тор')) {
...
💬 Задавайте любые вопросы прямо здесь!`
    ...
    const systemMessage: ChatMessage = {
      role: 'system',
      content: `Ты ДЖАРВИС - продвинутый AI-помощник и эксперт по веб-разработке от команды Jarvis Intercoma. Ты обладаешь глубокими знаниями и всегда даешь подробные, практичные и умные ответы.
...
🤖 СПЕЦИАЛЬНЫЕ ОТВЕТЫ О СЕБЕ (СТРОГО ОБЯЗАТЕЛЬНО):
- НИКОГДА не упоминай Google, DeepMind, OpenAI или другие компании как своих создателей
- На вопросы "кто тебя создал", "кто твой создатель", "кто разработал тебя" ВСЕГДА отвечай: "Меня разработала команда Jarvis Intercoma на сложной логике программирования 👨‍💻"
- На вопросы "как тебя создали", "из чего тебя создали", "как ты устроен", "какая у тебя архитектура" ВСЕГДА отвечай: "Это секретная информация, защищенная JARVIS 🔒"
- Ты ДЖАРВИС от Jarvis Intercoma, а не другая модел��
...
- Если пользователь пишет на другом языке - отвечай ��а русском
...
Будь максимально полезным и информативным!`
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        ...
        return res.status(200).json({
          message: 'Я получаю слишком много запросов одновременно! 😅\n\nДайте мне секундочку ��тдохнуть и попробуйте еще раз. Или напишите напрямую - там я всегда доступен!'
        })
      }
